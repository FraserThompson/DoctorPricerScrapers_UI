...
class ScraperApp extends React.Component {
      ...
      function (response) {
        if (response.data) {
          ...
          this.setState({
            errorActive: true,
            errorMessage:
              "Error: Could not fetch PHO list, server may be temporarily down.",
          });
        }
      }.bind(this)
    );
  ...
  getLogsList(name) {
    Utils.JsonReq(
      config.apiUrl + "/dp/api/logs/?source=" + name,
      null,
      "GET",
      function (response) {
  ...
  handleStop() {
    ...
    Utils.JsonReq(
      config.apiUrl +
        "/dp/task_status?task_id=" +
        this.state.selected.state.current_task_id +
        "&module=" +
        this.state.selected.props.module,
      null,
      "DELETE",
      function (res) {
        if (res.error) {
  ...
  handleScrape() {
    ...
    Utils.JsonReq(
      config.apiUrl + "/dp/scrape",
      { module: this.state.selected.props.module },
      "POST",
      function (res) {
        if (res.error) {
          ...
          this.state.selected.setState({
            state: "Scraping",
            current_task_id: json_res.task_id,
            timer: setInterval(
              this.updateTask.bind(this, this.state.selected),
              5000
            ),
          });
        }
  ...
  handleDelete() {
    Utils.JsonReq(
      config.apiUrl + "/dp/api/pho",
      { name: this.state.selected.props.title },
      "DELETE",
      function (res) {
        if (res.error) {
          console.log(res.error);
  ...
  updateTask(selected) {
    console.log(
      "Checking the status of " +
        selected.state.current_task_id +
        " from " +
  ...
  render() {
    return (
      <ThemeProvider theme={theme}>
        ...
        >
          <Typography variant="h4" sx={{color: "white"}}>DoctorPricer Stats</Typography>
          {this.state.username && (
        ...
        <div style={{ display: "flex" }}>
          ...
          <div style={this.rightColumn}>
            ...
            {!this.state.selected && this.state.averages && (
              <Box>
                <Stats data={this.state.averages} />
              </Box>
            )}
          </div>
        </div>
